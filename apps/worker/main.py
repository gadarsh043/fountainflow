...
def _post_progress(
    ...
    """Post a job progress update to the API server callback URL.

    Args:
        settings: Worker settings (used for callback URL + secret).
        job_id: Job being processed.
        stage: Current JobStage string.
    ...
    try:
        with httpx.Client(timeout=5.0) as client:
            client.post(
                f"{settings.api_callback_url}/jobs/{job_id}/progress",
                json=payload,
...
def process_job(
    ...
    try:
        ...
        _post_progress(settings, job_id, "completed", 100.0, f"Completed in {elapsed_ms}ms")
        ...
        _post_progress(settings, job_id, "failed", 0.0, f"Failed: {exc}")
